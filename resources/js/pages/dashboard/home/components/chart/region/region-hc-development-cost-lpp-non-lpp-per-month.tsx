...
type RegionLearningCostPerMonth = {
    january: RealizationLppNonLpp[];
    february: RealizationLppNonLpp[];
    march: RealizationLppNonLpp[];
    april: RealizationLppNonLpp[];
    may: RealizationLppNonLpp[];
    june: RealizationLppNonLpp[];
    july: RealizationLppNonLpp[];
    august: RealizationLppNonLpp[];
    september: RealizationLppNonLpp[];
    october: RealizationLppNonLpp[];
    november: RealizationLppNonLpp[];
    december: RealizationLppNonLpp[];
};
...
type RegionHCDevelopmentCostLppNonLppPerMonthProp = {
    selectedRegion: number;
};

export default function RegionHCDevelopmentCostLppNonLppPerMonth({
    selectedRegion,
}: RegionHCDevelopmentCostLppNonLppPerMonthProp) {
    const [isLoading, setIsLoading] = useState(false);
    const [data, setData] = useState<RegionLearningCostPerMonth>();
    ...
    useEffect(() => {
        const fetchOverviewData = async () => {
            ...
            try {
                const response = await axios.get<RegionLearningCostPerMonth>(
                    `/api/region/${selectedRegion}/realization/ptpn-group/month`,
                );
                ...
                series: [
                    {
                        ...
                        data: Object.values(data).map((val) => {
                            const stringCost = val.find(
                                (i) => i.is_ptpn_group,
                            )?.total_cost;

                            return parseInt(stringCost ?? '0') / 1000;
                        }),
                        marker: {
                            enabled: false,
                        },
