...
type RegionCostTraining = {
    name: string;
    total_cost: string;
};

type RegionHCDevelopmentCostTrainingProp = {
    selectedRegion: number;
};

export default function RegionHCDevelopmentCostTraining({
    selectedRegion,
}: RegionHCDevelopmentCostTrainingProp) {
    const [isLoading, setIsLoading] = useState(false);
    const [data, setData] = useState<RegionCostTraining[]>();
    ...
    useEffect(() => {
        const fetchOverviewData = async () => {
            ...
            try {
                const response = await axios.get<RegionCostTraining[]>(
                    `/api/region/${selectedRegion}/realization/training`,
                );
    ...
    const chartOptions: Highcharts.Options = useMemo(() => {
        if (data) {
            return {
                ...
                title: {
                    text: 'Overview Biaya per Pelatihan (Rp. 000)',
                },
                xAxis: [
                    {
