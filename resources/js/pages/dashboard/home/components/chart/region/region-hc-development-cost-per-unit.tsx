...
type RegionHCDevelopmentCostPerUnitProp = {
    selectedRegion: number;
};

type RegionCostPerUnit = {
    unit_name: string;
    total_cost: string;
};

export default function RegionHCDevelopmentCostPerUnit({
    selectedRegion,
}: RegionHCDevelopmentCostPerUnitProp) {
    const [isLoading, setIsLoading] = useState(false);
    const [data, setData] = useState<RegionCostPerUnit[]>();
    ...
    useEffect(() => {
        const fetchOverviewData = async () => {
            ...
            try {
                const response = await axios.get<RegionCostPerUnit[]>(
                    `/api/region/${selectedRegion}/realization/unit`,
                );
    ...
    const chartOptions: Highcharts.Options = useMemo(() => {
        if (data) {
            return {
                ...
                title: {
                    text: 'Biaya Pengembangan SDM per Unit (Rp. 000)',
                },
                xAxis: [
                    {
