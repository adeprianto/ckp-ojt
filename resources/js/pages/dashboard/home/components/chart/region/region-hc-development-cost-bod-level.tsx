...
type RegionCostBodLevel = {
    bod_level: number;
    total_cost: string;
};

type RegionHCDevelopmentCostBodLevelProp = {
    selectedRegion: number;
};

export default function RegionHCDevelopmentCostBodLevel({
    selectedRegion,
}: RegionHCDevelopmentCostBodLevelProp) {
    const [isLoading, setIsLoading] = useState(false);
    const [data, setData] = useState<RegionCostBodLevel[]>();
    ...
    useEffect(() => {
        const fetchOverviewData = async () => {
            ...
            try {
                const response = await axios.get<RegionCostBodLevel[]>(
                    `/api/region/${selectedRegion}/realization/bod-level`,
                );
                ...
                series: [
                    {
                        ...
                        data: [
                            parseInt(
                                data.find((val) => val.bod_level === 6)
                                    ?.total_cost ?? '0',
                            ) / 1000,
                            parseInt(
                                data.find((val) => val.bod_level === 5)
                                    ?.total_cost ?? '0',
                            ) / 1000,
                            parseInt(
                                data.find((val) => val.bod_level === 4)
                                    ?.total_cost ?? '0',
                            ) / 1000,
                            parseInt(
                                data.find((val) => val.bod_level === 3)
                                    ?.total_cost ?? '0',
                            ) / 1000,
                            parseInt(
                                data.find((val) => val.bod_level === 2)
                                    ?.total_cost ?? '0',
                            ) / 1000,
                            parseInt(
                                data.find((val) => val.bod_level === 1)
                                    ?.total_cost ?? '0',
                            ) / 1000,
                        ],
                        marker: {
