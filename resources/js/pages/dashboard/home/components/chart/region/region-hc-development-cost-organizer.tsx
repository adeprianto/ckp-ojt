...
type RegionCostOrganizer = {
    name: string;
    total_cost: string;
};

type RegionHCDevelopmentCostOrganizerProp = {
    selectedRegion: number;
};

export default function RegionHCDevelopmentCostOrganizer({
    selectedRegion,
}: RegionHCDevelopmentCostOrganizerProp) {
    const [isLoading, setIsLoading] = useState(false);
    const [data, setData] = useState<RegionCostOrganizer[]>();
    ...
    useEffect(() => {
        const fetchOverviewData = async () => {
            ...
            try {
                const response = await axios.get<RegionCostOrganizer[]>(
                    `/api/region/${selectedRegion}/realization/organizer`,
                );
    ...
    const chartOptions: Highcharts.Options = useMemo(() => {
        if (data) {
            return {
                ...
                title: {
                    text: 'Overview Biaya per Penyelenggara (Rp. 000)',
                },
                xAxis: [
                    {
                        // Use categories explicitly since we removed dataTable
                        categories: data.map((val) => val.name),
                        crosshair: true,
                    },
                ],
                yAxis: [
                ...
                series: [
                    {
                        ...
                        yAxis: 0,
                        data: data.map(
                            (val) => parseInt(val.total_cost ?? '0') / 1000,
                        ),
                        marker: {
                            enabled: false,
                        },
